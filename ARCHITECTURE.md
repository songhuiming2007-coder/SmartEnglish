...
厨房查词时不是随便返回的，有一套评分系统（全部取对数，否则词典里 30 亿次的 "the" 会碾压一切个人习惯）：

```
总分 = log10(词典频率)                          （压缩到 0~10 分）
     + min(你选过的次数, 10) × 0.7              （选 1 次 ≈ 5 倍频率优势，封顶防霸榜）
     + log10(公共语料词对频率) × 0.3            （温和的上下文信号，如 "of the"）
     + min(你的词对次数, 10) × 1.0              （你个人的搭配习惯，最强信号）
```

**用人话说：** 你选过 1 次的词，相当于词典频率高 5 倍；选过 5 次，相当于高 3000 倍。你经常打 "thank you"，那打完 "thank" 之后 "you" 会排第一，即使 "the" 在英语里更常见。

**主动 vs 默认：** 按数字键、点鼠标、移动高亮后选的词记 2 分（你明确表达了偏好）；空格/Tab 直接接受第一个只记 1 分（你只是接受了默认）。
...
**句首自动大写：** 每个词的第一个字母按下时，InputController 会读光标前的真实文本（最多 16 个字符）：前面是 `.` `?` `!`、换行或文档开头，就认定是句首，首字母自动大写。鼠标点到句子中间、切换输入框都能判断准确。读不到文本的应用退回内部状态推断。

**Caps Lock 消歧：** 单个大写字母 "H" 有歧义——Shift 打出的上屏 "Have"，Caps Lock 亮灯时打出的上屏 "HAVE"。
...
1. 密码输入框（系统属性标记）
2. 应用的 bundle ID（内置 27+ 个应用，用户可在 `~/Library/Application Support/SmartEnglish/blocked_apps.json` 里追加，内容为 bundle ID 字符串数组）
...
输入法启动时读这个文件，之后持续监听文件变化——保存即生效，不用切换输入法。JSON 是唯一事实源：从 JSON 里删掉的片语也会同步删除。之后你打 "addr"，候选窗第一个就是你的完整地址。选中后直接上屏，不做任何大小写转换。
...
| 撇号（词中） | 并入 composingText，支持直接打 can't、I'm |
| 数字 1-9 | 选第 N 个候选词（主动选择，学习权重 2） |
| Space | 选中当前高亮的候选词（默认第一个） |
| Enter | 原样上屏你打的字母（不查词典） |
| Tab | 接受第一个候选词（默认接受，学习权重 1） |
| Backspace | 删最后一个字母 |
| Escape | 取消输入，清除下划线 |
| ← → | 移动候选窗里的蓝色选中框 |
| 标点符号 | 先上屏 composing 文本，再输出标点；紧跟在选词自动空格后时，标点会前移吞掉空格（"hello ." → "hello. "） |
